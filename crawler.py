...
class Crawler:
    ...
    def scrape_company_details(self, company_data_html, which_case, name, address, email, tax, www_link):
        try:
            if which_case == 1 or 2:
                ...
                if company_data[0].find("p", itemprop="address") is not None:
                    company_address = company_data[0].find(
                        "p", itemprop="address")
                ...
                if company_data[0].find("a", itemprop="sameAs") is not None:
                    company_www_link = company_data[0].find(
                        "a", itemprop="sameAs")
    ...
    def scrape_offers_list(self, offer_html, session, name):
        try:
            if offer_html.select_one(Locator.company_profile1_link) is not None:
                ...
                page = session.get(company_link2)
                company_data_html = self.parse_html(page.content)
                which_case = 2
                ...
                page = session.get(company_link3)
    ...
    def scrape_job_offer_view(self, selenium_helper, job_offer_url, session, count, company_data_array, name, address, email, tax, www_link):
        ...
        if apply_button == '' or None:
            ...
            if "system.erecruiter.pl" in page.url:
                ...
                which_case, name, company_data_html = self.scrape_offers_list(
                    offer_html, session, name)
                (name, address, email, tax, www_link) = self.scrape_company_details(
                    company_data_html, which_case, name, address, email, tax, www_link)
                # ADD to array
                company_data_array.append((name, address, email, www_link))

    def scrape_job_tiles(self, count, session, company_data_array, job_tile, selenium_helper):
        name = ''
        address = ''
        email = ''
        tax = ''
        ...
        if tile_header[0].name == "a":
            ...
            self.scrape_job_offer_view(
                selenium_helper, job_offer_url, session, count, company_data_array, name, address, email, tax, www_link)
    ...
    def parse_tiles(self, session: requests.Session, company_data_array: list, selenium_helper: SeleniumHelper, starting_page: int) -> list:
        # try:
        # list view
        count = 1
        futures = []
